# -*- coding: utf-8 -*-
"""feature_extraction_SEL_for_(SEM_ELOC_DATASET)_(8_12_2021).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1TP4r2_xIAdECYNz-bk6ilhw8RAGqwSbV
...
lexicon = pd.read_csv(r"SEL lexicon with PFA.csv")
lexicon.head(5)
...
# if you want to apply stopword removal, set below variable to True
...
if stop_words_removal:
    ...
    stopwords = r'\b(?:{})\b'.format('|'.join(arb_stopwords)) #removes list of stopwords from our tweets
...
def calcExtraFeaturehappy(query):
    ...
    for i in query.split(" "):
        try:
            ...
            lexicon_score_happy += sc1
        except:
            # May be lexicon not available, just skip
            pass
        
    return lexicon_score_happy
...
df["lexicon_score_happy"] = df.apply(lambda row : calcExtraFeaturehappy(row['clean_tweet_stemmed']), axis = 1)

df["lexicon_score_sadness"] = df.apply(lambda row : calcExtraFeatureSad(row['clean_tweet_stemmed']), axis = 1)

df

# we save the new dataset after preprocessing and add feature extraction to use it later
df.to_csv('feature extraction with SEL (SEM-ELOC).csv', header=True, index=False, encoding='utf-8-sig')
