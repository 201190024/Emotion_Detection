# -*- coding: utf-8 -*-
"""(Deep_Learning)_Word2Vec_+_NRC__(21_12_2021)_(SEM_ELOC)_.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Yd0EzH-oHcwn7G09v1wUMEgwtXez7DeW
"""
...
# , "lexicon_score_trust", "lexicon_score_disgust", "lexicon_score_surprise", "lexicon_score_anticipation" ]].values
other_featuresNRC.shape

import numpy as np

all_feature = np.concatenate((tweets, other_featuresNRC), axis=1)
all_feature.shape
...
model1.add(layers.Embedding(len(word_index) + 1,
                            ...
                            input_length=max_len+4,
                            trainable=False))

# Adding LSTM layer
model1.add(layers.LSTM(64,dropout=0.1))
# model1.add(layers.LSTM(64,dropout=0.1))
...
history = model1.fit(X_train, y_train, epochs=10, batch_size=8, validation_data=(X_test, y_test),callbacks=[checkpoint1])
...
history = model2.fit(X_train, y_train, epochs=10, batch_size=8, validation_data=(X_test, y_test),callbacks=[checkpoint2])
...
model3.add(layers.Embedding(len(word_index) + 1,
                            ...
                            input_length=max_len+4,
                            trainable=False))
model3.add(layers.Conv1D(128, 8, activation='relu',kernel_regularizer=regularizers.l1_l2(l1=2e-3, l2=2e-3),bias_regularizer=regularizers.l2(2e-3)))
model3.add(layers.MaxPooling1D(5))
model3.add(layers.Conv1D(64, 8, activation='relu',kernel_regularizer=regularizers.l1_l2(l1=2e-3, l2=2e-3),bias_regularizer=regularizers.l2(2e-3)))
...
history = model3.fit(X_train, y_train, epochs=10, batch_size= 32,validation_data=(X_test, y_test), callbacks=[checkpoint3])
